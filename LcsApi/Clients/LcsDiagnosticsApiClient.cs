...
namespace LcsApi.Clients
{
    internal class LcsDiagnosticsApiClient : LcsApiClientBase
    {
        ...
        {
            List<KeyValuePair<string, object>> body = new()
            {
                new("startStr", startDate ?? CurrentTimeStamp),
                new("endStr", endDate ?? CurrentTimeStamp),
                new("serviceUnit", serviceUnit ?? string.Empty),
                new("maxPntsStr", limit)
            };

            if (metricNames is null)
            {
                body.Add(new("metrics[]", @"\Microsoft Dynamics AX Batch Server Group(total)\Batch Critical Job failed per second"));
                body.Add(new("metrics[]", @"\Microsoft Dynamics AX Database(axhost)\SQL deadlock errors as percentage of total"));
                body.Add(new("metrics[]", @"\Microsoft Dynamics AX Database(axhost)\SQL duplicate key exceptions as percentage of total"));
                body.Add(new("metrics[]", @"\Microsoft Dynamics AX Database(axhost)\SQL statements failed as percentage of total"));
                body.Add(new("metrics[]", @"\Microsoft Dynamics AX Database(axhost)\Slow SQL statements executed as percentage of total"));
                body.Add(new("metrics[]", @"\Memory\Available MBytes"));
                body.Add(new("metrics[]", @"\Processor(_Total)\% Processor Timen"));
            }
            else
            {
                foreach (string metricName in metricNames)
                {
                    body.Add(new("metrics[]", metricName));
                }
            }

            return await PostAsync<Dictionary<string, Dictionary<string, MetricData?>?>?>("ComponentMonitors/GetMetricData", projectId, 
                new() { { "environmentId", environmentId } }, body, URL_ENCODED_CONTENTTYPE, cancellationToken);
        }
        ...
        public async Task<FastQueryResult[]?> InvokeFastQueryAsync(int projectId, Guid environmentId, string queryName, string actionType = "Query", string? queryDescription = null, Dictionary<string, object>? parameters = null, CancellationToken cancellationToken = default)
        {
            var body = new List<KeyValuePair<string, object>>
            {
                ...
                new("queryDescription", queryDescription ?? string.Empty),
            ...
            { 
                ...
                {
                    body.Add(new(parameter.Key, parameter.Value));
                }
            }
            ...
            return rawResponse is not null ? JsonSerializer.Deserialize<FastQueryResult[]>(rawResponse) : default;
        }
        ...
        {
            Dictionary<string, object> additionalParameters = new()
            {
